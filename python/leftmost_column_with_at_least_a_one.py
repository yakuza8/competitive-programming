...
class Solution:
    ...
    def leftMostColumnWithOne(binaryMatrix: 'BinaryMatrix') -> int:
        ...
        """
        row_count, column_count = binaryMatrix.dimensions()
        cx, cy = 0, column_count - 1
        result_column = -1
        while cx < row_count and -1 < cy:
            if binaryMatrix.get(cx, cy) == 0:
                cx += 1
            else:
                result_column = cy
                cy -= 1
...
class LeftmostColumnWithAtLeastAOne(unittest.TestCase):

    def test_case_1(self):
        matrix = BinaryMatrix([[0, 0], [1, 1]])
        expected_result = 0
        self.assertEqual(expected_result, Solution.leftMostColumnWithOne(matrix))
