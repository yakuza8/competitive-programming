    ...
    def PredictTheWinner(nums: List[int]) -> bool:
        ...
        def helper(l, turn):
            if not l:
                return 0
            print(l, turn)
            choose_left = turn * l[0] + helper(l[1:], -turn)
            choose_right = turn * l[-1] + helper(l[:-1], -turn)
            return turn * max(turn * choose_left, turn * choose_right)

        return helper(nums, 1) >= 0


class PredictTheWinner(unittest.TestCase):

    def test_case_1(self):
        nums = [1, 5, 2]
        self.assertEqual(False, Solution.PredictTheWinner(nums))

    def test_case_2(self):
        nums = [1, 5, 233, 7]
        self.assertEqual(True, Solution.PredictTheWinner(nums))
