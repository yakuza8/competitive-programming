...
class Solution:
    ...
    def topKFrequent(words: List[str], k: int) -> List[str]:
        ...
        for word, count in counts.items():
            heapq.heappush(frequencies, (-count, word))

        for _ in range(k):
            ans.append(heapq.heappop(frequencies)[1])

        return ans


class TopKFrequentWords(unittest.TestCase):

    def test_case_1(self):
        words, k = ['i', 'love', 'leetcode', 'i', 'love', 'coding'], 2
        expected_result = ['i', 'love']
        self.assertListEqual(expected_result, Solution.topKFrequent(words, k))
